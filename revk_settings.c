...
// According to official documentation, we must supply a value of unsigned char or EOF
// to isXXXX(), however on all systems we know char is signed.
// Internally these are often implemented as macros, which index into a table of
// character properties. Supplying a negative value may cause random memory dereference
// by negative indexing.
// Some libc implementations went even further and employ some trick, which issues
// an "array subscript has type ‘char’" warning if isXXX() is called on an actual char
// argument. We're using these wrappers in order to avoid that. Inconvenient, but that's
// the C standard. Weird...
static int
is_digit (char c)
{
